  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (resultado?.ok) {
        ...
        // O middleware cuidará de redirecionar para a Roda da Vida se for primeiro acesso
        router.push('/')
        router.refresh()
      }
    } catch (error) {
      ...
      <div className="max-w-5xl w-full grid lg:grid-cols-2 gap-6">
        <div className="luxury-card-strong p-10 flex flex-col justify-between">
          <div>
            ...
            <h1 className="luxury-title text-5xl mb-3">Da Sombra à Luz</h1>
        ...
        <div className="luxury-card-strong p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={carregando}
              className="w-full luxury-btn-primary py-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {carregando ? (
                <>
          ...
          <div className="mt-6 flex items-center justify-between gap-4 text-sm">
            <p className="text-brand-medium">Não tem conta? Fale com o administrador.</p>
            <button
