...
export default function HistoricoPage() {
  ...
  function calcularMedia(avaliacao: Avaliacao): number {
    const valores = AREAS.map(area => avaliacao[area.key as keyof Avaliacao] as number)
    return valores.reduce((a, b) => a + b, 0) / valores.length
  }

  function calcularDiferenca(atual: number, anterior: number): { valor: number; percentual: number } {
    const valor = atual - anterior
    const percentual = anterior > 0 ? ((valor / anterior) * 100) : 0
  ...
  const avaliacaoAnterior = avaliacoesOrdenadas[1]
    ...
    <div className="luxury-shell py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <button
            onClick={() => router.push('/')}
            className="inline-flex items-center gap-2 text-brand-dark hover:text-brand-bronze transition-colors mb-4"
          >
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="font-tan-mon-cheri text-4xl text-brand-dark">
        ...
        {avaliacoes.length === 0 ? (
          <div className="luxury-card-strong p-12 text-center">
            ...
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-brand-bronze to-brand-gold text-white font-semibold rounded-xl hover:from-brand-dark hover:to-brand-medium transition-all shadow-lg hover:shadow-xl"
            >
              Fazer Avaliação
            </button>
          </div>
              ...
              <div className="luxury-card-strong p-8">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="bg-gradient-to-br from-brand-gold/10 to-brand-bronze/10 rounded-xl p-6 border border-brand-gold/30">
                    ...
                    <p className="text-xs text-brand-darker mb-4">
                      {new Date(avaliacaoMaisRecente.dataAvaliacao).toLocaleDateString('pt-BR')}
                    </p>
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 rounded-full bg-gradient-to-br from-brand-bronze to-brand-gold flex items-center justify-center shadow-lg">
                        <span className="text-2xl font-bold text-white">
                          {calcularMedia(avaliacaoMaisRecente).toFixed(1)}
                        </span>
                      </div>
                      <div>
                        <p className="text-xs text-brand-medium">Média Geral</p>
                        <p className="text-sm font-semibold text-brand-dark">de 10</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-gray-100 to-gray-50 rounded-xl p-6 border border-gray-300">
                    ...
                    <p className="text-xs text-gray-700 mb-4">
                      {new Date(avaliacaoAnterior.dataAvaliacao).toLocaleDateString('pt-BR')}
                    </p>
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 flex items-center justify-center shadow-lg">
                        <span className="text-2xl font-bold text-white">
                          {calcularMedia(avaliacaoAnterior).toFixed(1)}
                        </span>
                      </div>
                      <div>
                        <p className="text-xs text-gray-600">Média Geral</p>
                        <p className="text-sm font-semibold text-gray-700">de 10</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {AREAS.map((area) => {
                    const valorAtual = avaliacaoMaisRecente[area.key as keyof Avaliacao] as number
                    const valorAnterior = avaliacaoAnterior[area.key as keyof Avaliacao] as number
                    const diferenca = valorAtual - valorAnterior
                    
                    return (
                      <div key={area.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                        <div className="flex items-center gap-3 flex-1">
                          {renderIconeTendencia(diferenca)}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <span className={`text-sm font-semibold min-w-[60px] text-right ${
                            diferenca > 0 ? 'text-green-600' : diferenca < 0 ? 'text-red-600' : 'text-gray-400'
                          }`}>
              ...
              <div className="space-y-4">
                {avaliacoesOrdenadas.map((avaliacao, index) => (
                  <div
                    ...
                    onClick={() => router.push(`/resultado/${avaliacao.id}`)}
                  >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-brand-bronze to-brand-gold flex items-center justify-center shadow-md">
                          <span className="text-xl font-bold text-white">
                            {calcularMedia(avaliacao).toFixed(1)}
                          </span>
                        </div>
                        <div>
