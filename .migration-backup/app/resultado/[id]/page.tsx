...
export default function ResultadoPage() {
  ...
  useEffect(() => {
    const fetchDados = async () => {
      try {
        ...
        const responseAtual = await fetch(`/api/avaliacoes/${params.id}`)
        if (responseAtual.ok) {
          const data = await responseAtual.json()
          setAvaliacao(data)
          
          // Buscar todas avaliações do usuário
          const responseTodas = await fetch('/api/avaliacoes')
          if (responseTodas.ok) {
  ...
  // Função para calcular média de uma avaliação
  const calcularMedia = (aval: Avaliacao) => {
    const valores = [
      aval.plenitudeFelicidade,
      aval.espiritualidade,
      aval.saudeDisposicao,
      aval.desenvolvimentoIntelectual,
      aval.equilibrioEmocional,
      aval.familia,
      aval.desenvolvimentoAmoroso,
      aval.vidaSocial,
      aval.realizacaoProposito,
      aval.recursosFinanceiros,
      aval.contribuicaoSocial,
      aval.criatividadeHobbyDiversao,
    ]
  ...
  const evolucaoData = todasAvaliacoes.length > 1 ? {
    labels: todasAvaliacoes.map((a) => 
      format(new Date(a.dataAvaliacao), 'dd/MM', { locale: ptBR })
    ),
    datasets: [
      {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="luxury-card-strong p-8 md:p-12 animate-fadeIn">
          <div className="text-center mb-8">
            <h1 className="font-tan-mon-cheri text-5xl md:text-6xl text-brand-dark mb-2">
              Seu Resultado
            </h1>
            <p className="text-brand-medium text-lg">
              Confira o equilíbrio das suas áreas da vida
            </p>
          </div>
            ...
            <div className="grid md:grid-cols-3 gap-4 text-center">
              ...
              <div>
                ...
                <p className="font-semibold text-brand-dark">
                  {avaliacao.usuario.dataNascimento ? avaliacao.usuario.dataNascimento.split('-').reverse().join('/') : 'Não informado'}
                </p>
              </div>
              <div>
                <p className="text-sm text-brand-medium mb-1">Data da Avaliação</p>
                <p className="font-semibold text-brand-dark">
                  {format(new Date(avaliacao.dataAvaliacao), "dd/MM/yyyy 'às' HH:mm", {
                    locale: ptBR,
                  })}
                </p>
              </div>
                ...
                <div className="mt-6 grid md:grid-cols-3 gap-4">
                  <div className="bg-white rounded-xl p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-gray-800">
                      {calcularMedia(todasAvaliacoes[0]).toFixed(1)}
                    </p>
                    <p className="text-xs text-gray-500">
                      {format(new Date(todasAvaliacoes[0].dataAvaliacao), 'dd/MM/yyyy', { locale: ptBR })}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-green-100 to-emerald-100 rounded-xl p-4 text-center border-2 border-green-300">
                    ...
                    <p className="text-xs text-green-600">
                      {format(new Date(todasAvaliacoes[todasAvaliacoes.length - 1].dataAvaliacao), 'dd/MM/yyyy', { locale: ptBR })}
                    </p>
                  </div>
                  <div className="bg-white rounded-xl p-4 text-center">
                    <p className="text-sm text-gray-600 mb-1">Progresso Total</p>
                    <p className={`text-2xl font-bold ${
                      calcularMedia(todasAvaliacoes[todasAvaliacoes.length - 1]) >= calcularMedia(todasAvaliacoes[0])
                        ? 'text-green-600'
                        : 'text-amber-600'
                    }`}>
                      {(calcularMedia(todasAvaliacoes[todasAvaliacoes.length - 1]) - calcularMedia(todasAvaliacoes[0])) >= 0 ? '+' : ''}
                      {(calcularMedia(todasAvaliacoes[todasAvaliacoes.length - 1]) - calcularMedia(todasAvaliacoes[0])).toFixed(1)}
                    </p>
          ...
          <div className="bg-brand-gold/10 rounded-2xl p-6 mb-8 border border-brand-gold/30">
            <h3 className="font-semibold text-brand-dark mb-2">Sobre seu resultado</h3>
            <p className="text-brand-darker leading-relaxed">
              Este gráfico mostra como estão suas áreas da vida no momento. 
              As pontuações mais altas representam áreas equilibradas e desenvolvidas, 
              enquanto as menores indicam pontos de atenção onde você pode focar 
              seu crescimento pessoal.
            </p>
          </div>

          <div className="mb-8">
            <h3 className="font-tan-mon-cheri text-3xl text-brand-dark mb-4">
              Notas por Área
            </h3>
            <div className="grid md:grid-cols-2 gap-4">
              {AREAS_DA_VIDA.map((area, index) => {
                ...
                return (
                  <div
                    key={area.id}
                    className="bg-brand-gold/5 rounded-xl p-4 hover:shadow-md transition-all border border-brand-gold/20"
                  >
          ...
          <div className="flex flex-wrap gap-3 justify-center no-print">
            ...
            {!primeiroAcesso && (
              <button
                onClick={() => router.push('/avaliacao?novo=true')}
