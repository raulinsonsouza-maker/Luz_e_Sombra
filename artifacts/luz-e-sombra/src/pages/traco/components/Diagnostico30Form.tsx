...
export type Diagnostico30FormPayload = {
  respostas: EntradaDiagnostico30;
  resultado: ReturnType<typeof computarDiagnostico30>["diagnosticoEmocional"];
};

export type Diagnostico30FormProps = {
  pessoaId: number | null;
  /** `standalone` = página inteira com resultado e link para Traço; `embedded` = fluxo no Traço (sem tela de resultado, chama onConcluido). */
  variant?: "standalone" | "embedded";
  onConcluido?: (payload: Diagnostico30FormPayload) => void;
  onProgresso?: (passo: number, total: number) => void;
};

export default function Diagnostico30Form({
  pessoaId,
  variant = "standalone",
  onConcluido,
  onProgresso,
}: Diagnostico30FormProps) {
    ...
    try {
      ...
      if (localEnt) {
        ...
        if (isDiagnostico30RespostasCompletas(localEnt)) {
          const comp = computarDiagnostico30(localEnt);
          setComputado(comp);
      ...
      if (isDiagnostico30RespostasCompletas(r.data)) {
        const comp = computarDiagnostico30(r.data);
        setComputado(comp);
        if (variant === "standalone") {
          setFase("resultado");
        } else {
  ...
  useEffect(() => {
    if (fase === "perguntas") {
      onProgresso?.(idx + 1, totalPerguntas);
    }
  }, [fase, idx, onProgresso, totalPerguntas]);
  ...
  function avancar() {
    ...
    const payload: Diagnostico30FormPayload = {
      respostas: parsed.data,
      resultado: comp.diagnosticoEmocional,
    };
    try {
      localStorage.setItem(storageKey, JSON.stringify({ respostas: parsed.data, resultado: comp.diagnosticoEmocional }));
    } catch {
      /* ignore */
    }
  ...
  return (
    <div className={variant === "embedded" ? "" : "max-w-lg mx-auto px-4 pt-4 md:pt-8 pb-10"}>
        {fase === "intro" && (
          <section>
            {variant === "standalone" && (
              <>
        ...
        {fase === "resultado" && computado && variant === "standalone" && (
